...
from torch_geometric.nn import GraphConv, BatchNorm # GCNConv, SAGEConv, GATConv
# SAGEConv didn't work, so I use GraphConv instead
...
class GraphGAN:
    ...
    def compute_wgan_gp_loss(self, real_data, fake_data, lambda_rep, sigma, edge_index_1, edge_index_2, edge_index_3, edge_index_4, edge_index_5):
        ...
        # d_interpolated = self.discriminator(interpolated_data, real_data.edge_index)
    ...
    def train(self, num_epochs):
        ...
        for epoch in tqdm(range(num_epochs), desc="Training", mininterval=10):
            for data in self.data_loader:
                ...
                edge_index_1 = edge_indexes(batch_size, graph_nodes, 10)
                edge_index_2 = edge_indexes(batch_size, graph_nodes, 7)
                edge_index_3 = edge_indexes(batch_size, graph_nodes, 5)
                edge_index_4 = edge_indexes(batch_size, graph_nodes, 3)
                edge_index_5 = edge_indexes(batch_size, graph_nodes, 1)
                ...
                for _ in range(self.critic_it):
                    ...
                    # noise = torch.randn(batch_size, graph_nodes, self.input_dim).to(self.device)
                    with torch.no_grad():
                        fake_data = self.generator(noise, edge_index_1, edge_index_2, edge_index_3, edge_index_4, edge_index_5).to(self.device)

...
class Generator(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, dropout_rate=0.2):
        ...
        self.conv1 = GraphConv(input_dim, hidden_dim*8)
        # self.bn1 = nn.BatchNorm1d(hidden_dim*8)
        # self.bn1 = nn.LayerNorm(hidden_dim*8)
        self.bn1 = pyg_nn.norm.BatchNorm(hidden_dim*8)
        
        self.conv2 = GraphConv(hidden_dim*8, hidden_dim*4)
        # self.bn2 = nn.LayerNorm(hidden_dim*4)
        self.bn2 = pyg_nn.norm.BatchNorm(hidden_dim*4)

        self.conv3 = GraphConv(hidden_dim*4, hidden_dim*2)
        ...
        self.conv6 = GraphConv(output_dim, output_dim)
        # self.bn5 = nn.LayerNorm(output_dim)
        self.bn6 = pyg_nn.norm.BatchNorm(output_dim)


    def forward(self, x, edge_index_1, edge_index_2, edge_index_3, edge_index_4, edge_index_5, edge_index_6):
        ...
        x = self.conv3(x, edge_index_3)
        x = self.bn3(x)
        x = self.dropout(x)
        x = torch.relu(x)

        x = self.conv4(x, edge_index_4)
        x = self.bn4(x)
        x = self.dropout(x)
        x = torch.relu(x)

        x = self.conv5(x, edge_index_5)
        x = self.bn5(x)
        x = self.dropout(x)

        x = self.conv6(x, edge_index_6)
        x = self.bn6(x)
        # x = self.dropout(x)
...
class Discriminator(nn.Module):
    ...
    def forward(self, x, edge_index_1, edge_index_2, edge_index_3, edge_index_4, edge_index_5):
...
class GraphGAN:
    ...
    def compute_wgan_gp_loss(self, real_data, fake_data, lambda_rep, sigma, edge_index_1, edge_index_2, edge_index_3, edge_index_4, edge_index_5):
        ...
        d_interpolated = self.discriminator(interpolated_data, edge_index_1, edge_index_2, edge_index_3, edge_index_4, edge_index_5)
    ...
    def train(self, num_epochs):
        ...
        for epoch in tqdm(range(num_epochs), desc="Training", mininterval=10):
            for data in self.data_loader:
                ...
                edge_index_1 = edge_indexes(batch_size, graph_nodes, 1)
                edge_index_2 = edge_indexes(batch_size, graph_nodes, 3)
                edge_index_3 = edge_indexes(batch_size, graph_nodes, 5)
                edge_index_4 = edge_indexes(batch_size, graph_nodes, 7)
                edge_index_5 = edge_indexes(batch_size, graph_nodes, 10)     
                edge_index_6 = edge_indexes(batch_size, graph_nodes, 10)
                ...
                for _ in range(self.critic_it):
                    ...
                    with torch.no_grad():
                        fake_data = self.generator(noise, edge_index_1, edge_index_2, edge_index_3, edge_index_4, edge_index_5, edge_index_6).to(self.device)

                    grad_penalty = self.compute_wgan_gp_loss(real_data, fake_data, self.lambda_rep, self.sigma, 
                                                            edge_index_1, edge_index_2, edge_index_3, edge_index_4, edge_index_5)

                    critics_fake = self.discriminator(fake_data, edge_index_1, edge_index_2, edge_index_3, edge_index_4, edge_index_5).reshape(-1)
                    critics_real = self.discriminator(real_data.x, edge_index_1, edge_index_2, edge_index_3, edge_index_4, edge_index_5).reshape(-1)
